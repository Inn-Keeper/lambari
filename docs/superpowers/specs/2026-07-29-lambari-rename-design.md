...
Rename the project from its former name to Lambari across its source,
configuration, documentation, repository, and local checkout, leaving no
active former identity behind.
...
- Replace the former environment-variable prefix with `LAMBARI_ADDR` and
  `LAMBARI_KAFKA_BROKERS`.
- Rename the GitHub repository and local checkout directory to `lambari`.
...
- Search tracked and relevant untracked files case-insensitively for the former
  name; expect no product-identity references.
